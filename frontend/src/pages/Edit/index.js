...
function Edit() {
    ...
    let description = JSON.parse(params.get("description"));
    ...
    const [toProcessDump] = useState(true);
    ...
    useEffect(() => {
        axios.get(address + "/api/v1_0/" + type + "/" + description.id)
            ...
            .catch(e => NotificationProvider.createError(e.response.data.detail));
    }, [])
    ...
    const submitChanges = () => {
        let singular = type.substring(0, type.length - 1)
        console.log(singular)
        if (containTableData !== "/loader" && (removeTableData.length !== 0 || addTableData.length !== 0) && !errorText2) {
            axios.patch(address + `/api/v1_0/edit_${singular}/` + description.id, {
...
//            axios.patch(address + "/api/v1_0/cubes/" + description.id, {to_process: false, is_corrected: true})
//                .then(() => {setErrorText(""); setPage("/")})
//        }

        if (newQrCube && !errorText && type === "cubes") {
            axios.patch(address + "/api/v1_0/cubes/" + cubeId, {qr: newQrCube, to_process: false, is_corrected: true})
