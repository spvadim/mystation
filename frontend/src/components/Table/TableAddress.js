...
const TableAddress = React.memo(({
    ...
    const deleteRow = (id) => {
        axios.delete(baseAddress + "/api/v1_0/" + type + "/" + id)
            ...
            .catch(e => {setError(e.response.data); console.log(e.response.data.detail)})
    }
    if (loading) {
    ...
    return (
        <Table
            ...
            onEdit={row => history.push(`/edit?description=${JSON.stringify(row)}&type=${type}&extended=${extended}`)}
