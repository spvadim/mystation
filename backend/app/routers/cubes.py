...
from ..utils.background_tasks import add_cube_qr
...
async def create_cube_with_new_content(cube_input: CubeWithNewContent):
    ...
    batch = await engine.find_one(
        ProductionBatch,
        ProductionBatch.number.batch_number == batch_number,
    ...
    alarm_tp_message = f"Camera=Back|Event=CubeHandMade|DataQR={cube.qr}|AdditionalInfo=CubeId: {cube.id}"
...
async def finish_cube(qr: str):
    ...
    alarm_tp_message = f"Camera=Back|Event=CubeFinishManualBack|DataQR={cube.qr}|AdditionalInfo=CubeId: {cube.id}"
    await add_events("alarm_tp", alarm_tp_message)
    await engine.save(cube)
    return cube
...
async def add_qr_to_last_cube(qr: str, background_tasks: BackgroundTasks):
    if not await check_cube_qr(qr):
        raise HTTPException(400, detail=f"В таблице нет QR={qr}")
    ...
    cube.qr = qr
    tasks = [engine.save(cube), get_system_settings()]
    results = await asyncio.gather(*tasks)
    settings = results[1]
    signal_settings = settings.signal_settings
    background_tasks.add_task(add_cube_qr, signal_settings)
    return cube
...
async def edit_cube_by_id(id: ObjectId, edit_schema: CubeEditSchema):
    ...
    for _ in range(multipacks_in_cubes - len(multipack_ids_with_pack_ids)):
        multipack = Multipack(
            batch_number=batch_number,
            pack_ids=[],
            created_at=current_time,
        ...
        await engine.save(multipack)
        multipack_ids_with_pack_ids[str(multipack.id)] = []
    ...
    for k, v in multipack_ids_with_pack_ids.items():
        ...
        multipack_to_update.pack_ids = updated_multipack
    ...
    if edit_schema.is_corrected is not None:
        cube.is_corrected = edit_schema.is_corrected
    ...
    alarm_tp_message = (
        f"Camera=Back|Event=EditCube|DataQR={cube.qr}|AdditionalInfo=CubeId: {cube.id}"
    )
    await add_events("alarm_tp", alarm_tp_message)
    await engine.save(cube)
