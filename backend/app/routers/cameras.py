...
from ..utils.background_tasks import (
    ...
    drop_pack_after_pintset_snap7,
    drop_pack_ejector_erd,
    drop_pack_ejector_snap7,
    drop_signal,
...
async def new_pack_before_ejector(
    ...
    mode = await get_current_workmode()
    semi_auto = mode.work_mode == "semi-auto"
    current_datetime = await get_naive_datetime()
    data_qr = ""
    if mode.work_mode == "manual":
        raise HTTPException(400, detail="В данный момент используется ручной режим")
        ...
        error_msg = (
            "В данный момент используется полуавтоматический режим, пачка сброшена"
        )
        event = "ThrowPackSemiAutoWorkMode"
    ...
    elif pack.qr.startswith("multiple_qrs"):
        error_msg = f"{current_datetime} на камере перед сбрасывателем прошла пачка с несколькими QR"
        event = "ThrowPackMultipleQrs"

    elif not await check_qr_unique(Pack, pack.qr):
        error_msg = f"{current_datetime} на камере перед сбрасывателем прошла пачка с QR={pack.qr} и он не уникален в системе"
        event = "ThrowPackNonUniqueQR"
        data_qr = pack.qr

    if event:
        alarm_tp_message = f"Camera=Third|Event={event}|DataQR={data_qr}"
        await add_events("alarm_tp", alarm_tp_message)

    if error_msg:
        current_settings = await get_system_settings()

        if current_settings.general_settings.ejector_use_snap7.value:
            ...
            background_tasks.add_task(
                drop_pack_ejector_erd,
                error_msg,
                current_settings.second_erd_settings,
            )

        signal_settings = current_settings.signal_settings
        background_tasks.add_task(drop_signal, signal_settings)
...
async def pack_dropped_after_pintset(background_tasks: BackgroundTasks):
    ...
    alarm_tp_message = f"Camera=Back|Event=PackDropped|DataQR={pack_to_delete.qr}|AdditionalInfo=DroppedPackId: {pack_to_delete.id}"
    await add_events("alarm_tp", alarm_tp_message)
...
async def new_pack_after_pintset(
    ...
    next_pack_is_second = False
    to_process = False

    use_additional_event = current_settings.general_settings.use_additional_event.value
    data_qr = ""

    semi_auto = mode.work_mode == "semi-auto"

    if system_status.system_state.next_pack_is_second:
        next_pack_is_second = False
        if wait_second_pack:
            error_msg = "Стоит настройка сброса пары пачек одновременно. Эта пачка - вторая после первой плохой, она сброшена"
            if not pack.qr:
    ...
    if mode.work_mode == "manual":
    ...
    elif not pack.qr and not pack.barcode:
        to_process = True
        ...
        to_process = True
        error_msg = (
            f"{current_datetime} на камере за пинцетом прошла пачка с несколькими QR"
        )
        event = "ThrowPackMultipleQrs"
        if packs_under_pintset == 0:
            next_pack_is_second = True
    ...
    pack = Pack(qr=pack.qr, barcode=pack.barcode, to_process=to_process)
    ...
    system_status.system_state.next_pack_is_second = next_pack_is_second
    await engine.save(system_status)

    if event:
        alarm_tp_message = f"Camera=Counter|Event={event}|DataQR={data_qr}"
        if not use_additional_event:
            packs_to_drop = await get_packs_under_pintset()
            dropped_qrs = ",".join([pack.qr for pack in packs_to_drop])
            alarm_tp_message += f"|AdditionalInfo=Dropped qrs: {dropped_qrs}"
        await add_events("alarm_tp", alarm_tp_message)

    if error_msg and current_settings.general_settings.pintset_stop.value:
        not_drop = (next_pack_is_second and not semi_auto) and wait_second_pack
        if current_settings.general_settings.pintset_use_snap7.value and not not_drop:
            background_tasks.add_task(
                drop_pack_after_pintset_snap7,
                error_msg,
                current_settings.pintset_settings,
...
async def pintset_receive(background_tasks: BackgroundTasks):
    ...
    if delta < 0:
        ...
        old_packs_under_pintset = packs_under_pintset.copy()
        ...
        generated_packs = list(
            set([pack.qr for pack in packs_under_pintset])
            - set(pack.qr for pack in old_packs_under_pintset)
        )
        qrs = ",".join([qr for qr in generated_packs])
        alarm_tp_message = f"Camera=Pintset|Event=PintsetReceiveBack|DataQR={qrs}"
        await add_events("alarm_tp", alarm_tp_message)
    ...
    if delta > 0:
        ...
        if delta > 0:
            ...
            use_minvody_logic = (
                current_settings.general_settings.use_minvody_additional_logic.value
            )

            if use_minvody_logic:
                packs_under_pintset = packs_under_pintset[:-delta]

            elif delete_non_empty_packs:
        ...
        qrs = ",".join(str(pack.qr) for pack in packs_to_delete)
        alarm_tp_message = f"Camera=Pintset|Event=PintsetReceiveBack|DataQR={qrs}"
        await add_events("alarm_tp", alarm_tp_message)
...
async def pintset_finish(background_tasks: BackgroundTasks):
    ...
    if delta < 0:
        ...
        if delta < 0:
            if (
                ...
                qrs = ",".join([str(qr) for qr in generated_packs])
                alarm_tp_message = (
                    f"Camera=Pintset|Event=PintsetFinishBack|DataQR={qrs}"
                )
                await add_events("alarm_tp", alarm_tp_message)
    ...
    all_packs = [[] for i in range(multipacks_after_pintset)]

    for i in range(needed_packs):
        packs_on_assembly[i].in_queue = False
        all_packs[i % multipacks_after_pintset].append(packs_on_assembly[i])
    ...
    new_multipacks = []
    for packs in all_packs:
        multipack = Multipack(pack_ids=[pack.id for pack in packs])
        multipack.batch_number = number
        multipack.created_at = current_time
        multipack.to_process = bool([pack for pack in packs if pack.to_process])
...
async def multipack_enter_pitchfork_auto(background_tasks: BackgroundTasks):
    ...
    if multipacks_entered_pitchfork > multipacks_after_pintset * multiplier:
        ...
        await add_sync_error_to_bg_tasks(
            ...
            (
                f"На вилах более "
                f"{multipacks_after_pintset * multiplier} паллет:"
