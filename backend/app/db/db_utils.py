...
async def get_many_by_qr_or_404(model, qr: str) -> Model:
    instances = await engine.find(model, model.qr == qr, sort=query.desc(model.id))
    if not instances:
        raise HTTPException(404, detail=f"Не найдены элементы с QR={qr}")
    return instances
...
async def get_upper_aggregation_levels_by_pack_id(pack_id: ObjectId):
    id_model = IdModel(pack_id=pack_id)
    multipack = await engine.find_one(Multipack, {+Multipack.pack_ids: pack_id})
    if multipack:
        multipack_id = multipack.id
        id_model.multipack_id = multipack_id
        cube = await engine.find_one(
            Cube,
            {f"multipack_ids_with_pack_ids.{str(multipack_id)}": {"$exists": True}},
        )
        if cube:
            id_model.cube_id = cube.id
...
async def generate_packs(
    ...
    email_body = ""
    for i in range(n):
        datetime_string = current_datetime.strftime("%d.%m.%Y %H:%M:%S.%f")[:-3]
        new_pack = Pack(
            qr=f"generated_{generated_from} pack {datetime_string} {i}",
...
async def get_report(
    q: ReportRequest, include_red_cubes: bool = False
) -> ReportResponse:
    ...
    if include_red_cubes:
        cubes_query = query.and_(Cube.created_at < dt_end, Cube.created_at >= dt_begin)
    else:
        cubes_query = query.and_(
            Cube.created_at < dt_end,
            Cube.created_at >= dt_begin,
            Cube.to_process == False,
        )
